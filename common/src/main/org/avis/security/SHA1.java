...
/*
 * Modified version of SHA1 class from URL below. Removed dependence on Sun
 * JCE.
 * 
 * http://www.limewire.org/fisheye/viewrep/~raw,r=1.2/limecvs/core/com/limegroup/gnutella/SHA1.java 
 * 
 ...
 * SHA-1 and SHA-256) or < 2^^128 bits (for SHA-384 and SHA-512) is input to an
 * algorithm, the result is an output called a message digest. The message
 ...
 * 
 * <p>
 * The four hash algorithms specified in this "SHS" standard are called secure
 * because, for a given algorithm, it is computationally infeasible 1) to find a
 * message that corresponds to a given message digest, or 2) to find two
 * different messages that produce the same message digest. Any change to a
 * message will, with a very high probability, result in a different message
 * digest. This will result in a verification failure when the secure hash
 * algorithm is used with a digital signature algorithm or a keyed-hash message
 * authentication algorithm.
 * </p>
 * 
 * <p>
 * A "SHS change notice" adds a SHA-224 algorithm for interoperability, which,
 * like SHA-1 and SHA-256, operates on 512-bit blocks and 32-bit words,
 * </p>
 * 
 * <p>
 * <b>References: </b>
 * </p>
 * <ol>
 * <li>NIST FIPS PUB 180-2, "Secure Hash Signature Standard (SHS) with change
 * notice", National Institute of Standards and Technology (NIST), 2002 August
 * 1, and U.S. Department of Commerce, August 26. <br>
 ...
 * </ol>
 */
public final class SHA1 implements Cloneable
{
  ...
  /**
   * Private context for incomplete blocks and padding bytes. INVARIANT: padding
   * must be in 0..63. When the padding reaches 64, a new block is computed, and
  ...
  /**
   * Private contextual byte count, sent in the next block, after the ending
   * padding block.
   */
  private long bytes;
   ...
   * 
   * Can be used to allocate your own output buffer when computing multiple
   * digests.
   * 
  ...
  /**
   * Updates the digest using the specified byte. Requires internal buffering,
   * and may be slow.
   * 
   ...
   * specified offset.
   * 
   * Input length can be any size. May require internal buffering, if input
   * blocks are not multiple of 64 bytes.
   * 
   * Overrides the protected abstract method of java.security.MessageDigestSpi.
   * @param input
   ...
   * padding. Computes the final hash and returns the final value as a byte[20]
   * array. Once engineDigest has been called, the engine will be automatically
   * reset as specified in the JavaSecurity MessageDigest specification.
   * 
   * For faster operations with multiple digests, allocate your own array and
   * use engineDigest(byte[], int offset, int len).
   * 
   * Overrides the protected abstract method of java.security.MessageDigestSpi.
  ...
  /**
   * Completes the hash computation by performing final operations such as
   * padding. Once engineDigest has been called, the engine will be
   * automatically reset (see engineReset).
   * 
   * Overrides the protected abstract method of java.security.MessageDigestSpi.
   * @param hashvalue
  ...
  public int engineDigest (byte [] hashvalue, int offset, int len)
  {
    ...
    switch (temp)
    { // INVARIANT: temp must be in [0..63]
      case 56:
      ...
      default: // Clear the rest of 56 first bytes of pad[].
        switch (temp & 3)
        {
          case 3:
            ...
            break;
          case 2:
            pad [(temp += 2) - 1] = (byte)0x00;
            break;
          case 1:
            pad [(temp += 3) - 2] = (byte)0x00;
            pad [temp - 1] = (byte)0x00;
            break;
          case 0:
            pad [(temp += 4) - 3] = (byte)0x00;
            pad [temp - 2] = (byte)0x00;
            pad [temp - 1] = (byte)0x00;
        }
        do
  ...
  /**
   * Updates the digest using the specified array of bytes, starting at the
   * specified offset, but an implied length of exactly 64 bytes.
   * 
   * Requires no internal buffering, but assumes a fixed input size, in which
   * the required padding bytes may have been added.
   * 
   * @param input
   *          the array of bytes to use for the update.
   * @param offset
  ...
  private void computeBlock (final byte [] input, int offset)
  {
    ...
    /*
     * Cache the input block into the local working set of 64-bit values, in
     * big-endian byte order. Be careful when widening bytes or integers due to
     * sign extension !
     */
    int i00, i01, i02, i03, i04, i05, i06, i07, i08, i09, i10, i11, i12, i13, i14, i15;

    /*
     * Use hash schedule function Ch (rounds 0..19): Ch(x,y,z) = (x & y) ^ (~x &
     * z) = (x & (y ^ z)) ^ z, and K00 = .... = K19 = 0x5a827999.
     */
    /*
    ...
    /*
     * Use hash schedule function Parity (rounds 20..39): Parity(x,y,z) = x ^ y ^
     * z, and K20 = .... = K39 = 0x6ed9eba1.
     */
    ...
    /*
     * Use hash schedule function Maj (rounds 40..59): Maj(x,y,z) = (x & y)^(x &
     * z)^(y & z) = ((x & y)|((x | y)& z)), and K40 = .... = K59 = 0x8f1bbcdc.
     */
    ...
    /*
     * Use hash schedule function Parity (rounds 60..79): Parity(x,y,z) = x ^ y ^
     * z, and K60 = .... = K79 = 0xca62c1d6.
     */
    ...
    /*
     * Terminate the last two rounds of fifth pass, feeding the final digest on
     * the fly.
     */
    hB += b += ((c << 5) | (c >>> 27)) + 0xca62c1d6 // K78
        + (d ^ (e = (e << 30) | (e >>> 2)) ^ a) // Parity(d,e,a)
        + (i14 = ((i14 ^= i00 ^ i06 ^ i11) << 1) | (i14 >>> 31)); // W78
    hA += a += ((b << 5) | (b >>> 27)) + 0xca62c1d6 // K79
        + (c ^ (d = (d << 30) | (d >>> 2)) ^ e) // Parity(c,d,e)
